...
class MiniRpcProvider {
  constructor(chainId, url, batchWaitTimeMs) {
    ...
    this.clearBatch = async () => {
      // this hack preallocates memory for something down below,
      // note: using let fucks everything up by adding overhead
      var _a, _b, _c;
      ...
      for (const result of json) {
        ...
        if (resolve && reject) {
          if ("error" in result) {
            // currently inlining since function call overhead actually has negative
            // impact on performance but there should be way to pregen/ make code pretty
            reject(
              new RequestError(
                (_a =
